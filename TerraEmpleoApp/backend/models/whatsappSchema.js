...
async function initWhatsappSchema() {
  ...
  // Control de mensaje de seguimiento al empleador (recordatorio semanal "¿ya contrató?").
  try { await query('ALTER TABLE vacantes ADD COLUMN whatsapp_seguimiento_at TIMESTAMP NULL DEFAULT NULL'); } catch (_) {}
  // Cuántos recordatorios se enviaron (tope para no spamear) y si el empleador ya dijo "contraté".
